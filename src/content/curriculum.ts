...
export const curriculum: Curriculum = {
  ...
  projects: projects.map((project, order) => ({ id: project.id, path: `projects/${project.id}`, title: project.title, order: order + 1, status: order <= 1 ? 'published' as const : 'draft' as const, description: project.description, prerequisiteProjectIds: order === 0 ? [] : [projects[order - 1].id], topicIds: project.topics.map((topic) => topic.id) })),
  topics: projects.flatMap((project) => project.topics.map((outline, index) => {
    ...
    const authored = [...todoPhase3Topics, ...weatherTopics].find(topic => topic.id === outline.id);
