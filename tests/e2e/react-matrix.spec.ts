...
import { reactBasicReferenceByTopic, reactReferenceByTopic, reactStarter } from '../../src/content/reactReferences';
...
test.describe('Phase 5 React dashboard behavioral matrix', () => {
  ...
  test('all six React topics accept references and reject actual unfinished starters', async ({ page }) => {
    for (const topic of topics) for (const suffix of suffixes) {
      ...
      const referenceResult = await grade(page, reactReferenceByTopic[topic], id);
      expect.soft(referenceResult, id + ': ' + referenceResult.message).toMatchObject({ assessmentId: id, passed: true });
  ...
  test('first-increment references pass their own checks and fail the full contract', async ({ page }) => {
    for (const topic of topics) {
      for (const suffix of ['guided-1', 'apply'] as const) {
        const id = `${topic}-${suffix}`;
        await expect.soft(grade(page, reactBasicReferenceByTopic[topic], id), id).resolves.toMatchObject({ assessmentId: id, passed: true });
      }
      for (const suffix of ['guided-2', 'debug', 'combine'] as const) {
        const id = `${topic}-${suffix}`;
        await expect.soft(grade(page, reactBasicReferenceByTopic[topic], id), id).resolves.toMatchObject({ assessmentId: id, passed: false });
      }
    }
  });
  ...
  test('observed regressions fail without relying on source matching', async ({ page }) => {
    ...
    const mutations: Array<[string, string, Files]> = [
      ...
      ['effects omit cancellation', 'react-effects-persistence-and-errors', replace(refs['react-effects-persistence-and-errors'], 'App.jsx', 'return () => { latest.current += 1; controller.abort(); };', 'return () => {};')],
      ...
      ['smoke only prints pass', 'react-testing-and-export', replace(refs['react-testing-and-export'], 'App.jsx', 'onClick={runSmoke}', "onClick={() => setSmokeStatus('Smoke test passed.')}")],
    ];
