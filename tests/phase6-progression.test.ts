...
describe('Phase 6 cumulative mastery and existing session preservation', () => {
  for (const topic of curriculum.topics.filter(t => t.projectId === 'amazon-inspired-ecommerce')) {
    it(`${topic.id}: repeated reveals, misses and resume still allow ten distinct correct credits`, () => {
      ...
      if (successor) expect(isTopicUnlocked(successor, session, curriculum)).toBe(true); else expect(topic.id).toBe('shop-testing-export-and-handoff');
    });
  }
